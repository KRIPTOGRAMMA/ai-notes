...
<div class="wrap">
  <div class="card box">
    ...
    {:else if step === 3}
      ...
      <p>По умолчанию активность отслеживается, только когда окно приложения в фокусе.</p>
      <p>Для полного трекинга в фоне добавь свой аккаунт в группу <code>input</code>:</p>
      <pre>sudo usermod -aG input $USER</pre>
      <p class="muted" style="font-size:13px;">Потребуется перелогин. Этот шаг можно пропустить и вернуться к нему позже.</p>
    {:else if step === 4}
