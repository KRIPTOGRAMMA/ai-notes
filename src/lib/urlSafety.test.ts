...
describe("isSafeUrl", () => {
  it("обычные http/https ссылки безопасны", () => {
    ...
    expect(isSafeUrl("  https://example.com  ")).toBe(true); // с пробелами
  });
  ...
  // Главное, ради чего модуль существует: markdown в заметке — произвольный
  // текст (вставка из буфера, ответ ИИ, импортированный .md).
  ...
  it("относительные ссылки и мусор — не открываем", () => {
    expect(isSafeUrl("example.com")).toBe(false); // нет схемы
